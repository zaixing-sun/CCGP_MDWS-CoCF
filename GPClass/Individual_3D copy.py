...
from Environments.CLOUDFOGsystem import CloudFogSystem,InstanceList,Configuration_Container
...
def treeNode_R(tree, index, terminal):   
    ...
    #     pass
    try:
        tree_String = tree_String.replace("'MINCPU'",str(terminal.MINCPU))
    except:
        pass
    try:
        tree_String = tree_String.replace("'MINMEMORY'",str(terminal.MINMEMORY))
    except:
        pass
    try:
        tree_String = tree_String.replace("'TASKCATEGORY'",str(terminal.TASKCATEGORY))
    except:
        pass
    try:
        tree_String = tree_String.replace("'CURRENTLYCPU_INSTANCE'",str(terminal.CURRENTLYCPU_INSTANCE))
    except:
        pass
    try:
        tree_String = tree_String.replace("'CURRENTLYMEMORY_INSTANCE'",str(terminal.CURRENTLYMEMORY_INSTANCE))
    except:
        pass
    try:
        tree_String = tree_String.replace("'MINCPU_INSTANCE'",str(terminal.MINCPU_INSTANCE))
    except:
        pass
    try:
        tree_String = tree_String.replace("'MINMEMORY_INSTANCE'",str(terminal.MINMEMORY_INSTANCE))
    except:
        pass
    try:
        tree_String = tree_String.replace("'MAXCPU_INSTANCE'",str(terminal.MAXCPU_INSTANCE))
    except:
        pass
    try:
        tree_String = tree_String.replace("'MAXMEMORY_INSTANCE'",str(terminal.MAXMEMORY_INSTANCE))
    except:
        pass
    try:
        tree_String = tree_String.replace("'MINCOST_INSTANCE'",str(terminal.MINCOST_INSTANCE))
    except:
        pass
    try:
        tree_String = tree_String.replace("'MAXCOST_INSTANCE'",str(terminal.MAXCOST_INSTANCE))
    except:
        pass
    try:
        tree_String = tree_String.replace("'MINENERGY_INSTANCE'",str(terminal.MINENERGY_INSTANCE))
    except:
        pass
    try:
        tree_String = tree_String.replace("'MAXENERGY_INSTANCE'",str(terminal.MAXENERGY_INSTANCE))  
    except:
        pass
    try:
        tree_String = tree_String.replace("'MINUTILISATION_INSTANCE'",str(terminal.MINUTILISATION_INSTANCE))
    except:
        pass
    try:
        tree_String = tree_String.replace("'MAXUTILISATION_INSTANCE'",str(terminal.MAXUTILISATION_INSTANCE))
    except:
        pass
...
class Individual:
    def __init__(self): 
        ...
        self.multiWorkflows = []
        self.MCSPSystem = CloudFogSystem()  # []
            ...
            for eachPlstformID in range(len(self.MCSPSystem.platform)):  
                if self.MCSPSystem.platform[eachPlstformID].CloudProvider in self.MCSPSystem.Category[0].CloudProvider:
                    ...
                    for i in range(len(self.MCSPSystem.platform[eachPlstformID].VMPlatform)):  #  have been created
                        ...
                        availableServiceInstance[-1].append(temp_Instance)
                    ...
                    availableCloud.append(deepcopy(InstanceList(CloudProvider=self.MCSPSystem.platform[eachPlstformID].CloudProvider,TerminalCloud = INSTANCETERMINAL_dict())))  #  {0:INSTANCETERMINAL_dict(),1:None}
                    ...
                    availableCloud[-1].TerminalCloud.AVERAGESLACKTIME = availableCloud[-1].TerminalCloud.AVERAGEACTUALAVAILABLETIME - task_deadline
