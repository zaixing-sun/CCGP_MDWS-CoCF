...
from GPClass.function_terminal import WORKFLOWTERMINAL_dict,TASKTERMINAL_dict # ,CLOUDPLATFORMTERMINAL_dict
...
def readWorkflowData(fileName):
    WfW = workflowClass()
    WfW.name = fileName[0:-4]
    WfW.DAG = np.load('data_npy/'+fileName,allow_pickle=True).item()
    seed(os.path.getsize('data_npy/'+fileName))
    WfW.releaseTime = 100*random()  # randint(0,100)  ##### 随机生成工作流的到达时间
    try:
        WfW.deadlineFactor = WfW.DAG.pop('DeadlineFactor')   
    except:
    ...
    while True:
        ...
        for taskid in scheduleOrder:
            ...
            if parents != []:
                ...
                if boolean1:
                    continue
                listPEST = [ EST[each.id] + MET[each.id] + each.size/MULTICLOUDsystem.bandwidth_in1Cloud for each in parents  ] #GlobalResource.maxB
...
def getLFT(workflow,MET,Deadline,scheduleOrder):
    # scheduleOrder = breadth_first_search_SubDeadline(workflow)
    ...
    while True:
        ...
        for taskid in scheduleOrder:
            ...
            if child_1 != []:
                ...
                if boolean1:
                    continue                    
                listCLFT = [ (LFT[each.id] - MET[each.id] - each.size/MULTICLOUDsystem.bandwidth_in1Cloud) for each in child_1  ] # GlobalResource.maxB
...
def getRank_u(workflow,MET,scheduleOrder):
    # scheduleOrder = breadth_first_search_SubDeadline(workflow)
    scheduleOrder.reverse()
    ...
    while True:
        ...
        for taskid in scheduleOrder:
            ...
            if child_1 != []:
                ...
                if boolean1:
                    continue
                Rank_u[taskid] = MET[taskid] + max([(Rank_u[each.id] + each.size/MULTICLOUDsystem.bandwidth_in1Cloud) for each in child_1] ) # GlobalResource.maxB
            else:
                Rank_u[taskid] = MET[taskid]
            scheduleOrder.remove(taskid)
            break
    return Rank_u
...
def getSubDeadline(workflow,releaseTime,Deadline,DeadlineFactor):
    ...
    LFT,LST = getLFT(workflow,MET,Deadline,deepcopy(scheduleOrder))
    Rank_u = getRank_u(workflow,MET,deepcopy(scheduleOrder))
    for taskid,task in workflow.items():
        ...
        aveCT = np.average( [each.size/MULTICLOUDsystem.bandwidth_in1Cloud for each in workflow[taskid].outputs] ) if len(workflow[taskid].outputs)!= 0 else 0   # GlobalResource.maxB
