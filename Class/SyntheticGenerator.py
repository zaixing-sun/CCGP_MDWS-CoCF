...
class SyntheticGenerator():
    ...
    def generateSyntheticWorkFlow(self):
        ...
        tree = ET.parse('SyntheticWorkflows/'+self.fileName)
        root = tree.getroot()
        ...
        for job in root.findall('{http://pegasus.isi.edu/schema/DAX}job'):
            ...
            taskID = int(job.get('id')[2:])
            dict_task.setdefault(taskID,k)
            task = Task(id=dict_task[taskID], namespace=job.get('namespace'),name=job.get('name'), runtime=randomRuntime*SECONDS)
            # for file in files:
            #     randomSize = float(file.get('size'))/MB # GB # random.randint(MINDATA, MAXDATA)  #random.randint(10, 20) if random.random()<0.5 else random.randint(1, 9)
            ...
            #         task.addInput(tin)
            workFlow.update({dict_task[taskID]:task})
            totaltime += randomRuntime
            k += 1
        ...
        random.seed(int(totaltime*10000000))
        ...
        for child in root.findall('{http://pegasus.isi.edu/schema/DAX}child'):
            ...
            for parent in parents:
                parID = int(parent.get('ref')[2:])
                chiID = int(child.get('ref')[2:])
                fileSize = random.randint(MINDATA,MAXDATA)  
                # print(self.fileName,parID,chiID,fileSize) 
                
                tin = File(str(dict_task[parID]),id=dict_task[parID],size=fileSize,  booleaninput= False, booleanoutput= True)
                workFlow[dict_task[chiID]].addInput(tin) 
                
                tout = File(str(dict_task[chiID]),id=dict_task[chiID],size=fileSize, booleaninput= True, booleanoutput= False)
                workFlow[dict_task[parID]].addOutput(tout)                    
                # tin = File(str(parID),id=parID,size=fileSize,  booleaninput= False, booleanoutput= True)
                # workFlow[chiID].addInput(tin)
                # tout = File(str(chiID),id=chiID,size=fileSize, booleaninput= True, booleanoutput= False)
                # workFlow[parID].addOutput(tout)
        ...
        list1 = [i for i in range(len(workFlow))]
        listkey = list(workFlow.keys())
        if list1 != listkey:
            print()
